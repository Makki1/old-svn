/*
xpl-eibd.c -- Simple xPL eibd gateway
...
void printUsage(String ourName) {
  ...
  fprintf(stderr, "  -xpldebug -- enable xPLLib debugging messages\n");
...
void encodeEIS5 (double eis5value, char *buf)
{
    /*char buf[3];*/
    int ex = 0;
    writeDebug("eis5val1 %f", eis5value);
    int m = (eis5value * 100);
    writeDebug("eis5val2 %f", eis5value);
    if (m < 0)
    {
        m = -m;
        while (m > 2048)
...
void myxPLMessageHandler(xPL_MessagePtr theMessage, xPL_ObjectPtr userValue)
{
  ...
  if (!strcmp(xPL_getSchemaClass(theMessage),"control") &&
      ...
      xPL_getMessageType(theMessage)==xPL_MESSAGE_COMMAND )
      {
      /* check for vals without DPT, assume default DPT1 */
      if (!xPL_doesMessageNamedValueExist(theMessage,"dpt") &&
          atoi(xPL_getMessageNamedValue(theMessage, "current")) < 64 &&
          atoi(xPL_getMessageNamedValue(theMessage, "current")) > -1) 
          {
          xPL_setMessageNamedValue(theMessage,"dpt","1");
          } 
      /* write to eib */
      if (xPL_doesMessageNamedValueExist(theMessage,"device") &&
        xPL_doesMessageNamedValueExist(theMessage,"current") &&
        xPL_doesMessageNamedValueExist(theMessage,"type") &&
        xPL_doesMessageNamedValueExist(theMessage,"dpt") )
        {
        ...
        /* now select by either type or length OR TODO: GA-config */
            if (xPL_getMessageNamedValue(theMessage, "dpt"))
            dest = readgaddr (xPL_getMessageNamedValue(theMessage, "device"));
            switch (atoi(xPL_getMessageNamedValue(theMessage, "dpt")))
            {
            case 1: /* DPT1 / EIS 1 1bit */
                buf[1] |= atoi(xPL_getMessageNamedValue(theMessage, "current")) & 0x3f;
                len = EIBSendGroup (con, dest, 2, buf);
                if (len == -1)
                  writeError("EIBSend Request failed");
                writeDebug("EIBSend request %i",len);
                break;
            case 5: /* DPT5/EIS6 8bit unsigned */
                if (strstr(xPL_getMessageNamedValue(theMessage, "current"),"%")) {
                    /* percent value cut off */
                    /* xPL_getMessageNamedValue(theMessage, "current")[strlen(xPL_getMessageNamedValue(theMessage, "current"))] = "\0"; */
                    writeDebug("DPT5 val %s",xPL_getMessageNamedValue(theMessage, "current"));
                    buf[2] = (strtol(xPL_getMessageNamedValue(theMessage, "current"),NULL,0)*255/100) & 0xff;
                } else {
                    buf[2] = strtol(xPL_getMessageNamedValue(theMessage, "current"),NULL,0) & 0xff;
                }
                len = EIBSendGroup (con, dest, 3, buf);
                if (len == -1)
                  writeError("EIBSend Request failed");
                writeDebug("EIBSend request %i",len);
                break;
            case 9: /* EIS 5 float */
                encodeEIS5(strtod(xPL_getMessageNamedValue(theMessage, "current"),NULL),buf);
                len = EIBSendGroup (con, dest, 4, buf);
                if (len == -1)
                  writeError("EIBSend Request failed");
                writeDebug("EIBSend request %i",len);
                break;
            case 12: /* DPT12/EIS11 4byte unsigned */
                value = strtol(xPL_getMessageNamedValue(theMessage, "current"),NULL,0);
                buf[2] = ((value & 0xff000000)>>24);
                buf[3] = ((value & 0xff0000)>>16);
                buf[4] = ((value & 0xff00)>>8);
                buf[5] = (value & 0xff);
                len = EIBSendGroup (con, dest, 6, buf);
                if (len == -1)
                  writeError("EIBSend Request failed");
                writeDebug("EIBSend request %i",len);
                break;
            case 16: /* DPT16/EIS15 14byte char */
                for (i = 0; i <14; i++) {
                    if (strlen(xPL_getMessageNamedValue(theMessage, "current")) > i) {
                      buf[i+2] = xPL_getMessageNamedValue(theMessage, "current")[i];
                    } else {
                      buf[i+2] = 0;
                    }
                }
                len = EIBSendGroup (con, dest, 16, buf);
                if (len == -1)
                  writeError("EIBSend Request failed");
                writeDebug("EIBSend request %i",len);
                break;
            default:
                writeInfo("xPL Message to %s val %s dpt %s unknown DPT",xPL_getMessageNamedValue(theMessage, "device"),xPL_getMessageNamedValue(theMessage, "current"),xPL_getMessageNamedValue(theMessage, "dpt"));
                
            }
        }
      }
  /* sensor.request read-request to bus */
  if (!strcmp(xPL_getSchemaClass(theMessage),"sensor") &&
      !strcmp(xPL_getSchemaType(theMessage),"basic") &&
      xPL_getMessageType(theMessage)==xPL_MESSAGE_COMMAND &&
      xPL_doesMessageNamedValueExist(theMessage,"request") &&
      xPL_doesMessageNamedValueExist(theMessage,"device") )
      {
        buf[0] = 0;
        buf[1] = 0;
        dest = readgaddr (xPL_getMessageNamedValue(theMessage, "device"));
        ...
        if (len == -1)
          writeError("EIBSend ReadRequest failed");
        writeDebug("EIBSend ReadRequest %i",len);
      }
  /* xpl-trig datetime.basic DATETIME=<date and time as yyyymmddhhmmss> */
  /* DPT10/EIS 3 time */
  /* DPT11/EIS 4 date*/
