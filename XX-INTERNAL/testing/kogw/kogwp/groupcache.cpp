...
void
GroupCache::remove (grpaddr_t addr)
{
...
GroupCacheEntry
  GroupCache::Read (grpaddr_t addr, unsigned Timeout, uint16_t age)
{
...
Array < grpaddr_t > GroupCache::LastUpdates (uint16_t start, uint8_t Timeout,
					     uint16_t & end)
{
  Array < grpaddr_t > a;
  //pth_event_t timeout = pth_event (PTH_EVENT_RTIME, pth_time (Timeout, 0));
