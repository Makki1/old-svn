 ...
 *
 * VERSION 1.3 DS2423  for ATTiny25/2313A/ATmega48
 * Atmega not tested fully yet?!
 *
...
/* PORTB |= ( (1<<PB1) | (1<<PB2) | (1<<PB3) | (1<<PB4) ); */ /* Interne Pull-Up fuer beide einschalten */
...
const uint8_t owid[8]= {0x1D, 0x48, 0x48, 0x48, 0x00, 0x00, 0x01, 0x62};
...
int main(void) {
	...
	sei();
	    DDRD = ( 1 << PD6 );        // PD6 an PORTD als Ausgang setzen
		
	while(1){
		PORTD ^= ( 1 << PD6 );  // Toggle PD6 z.B. angeschlossene LED
//		_delay_ms(500);
	}
}	
