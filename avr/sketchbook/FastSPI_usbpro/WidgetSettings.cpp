 ...
 *   device label size (2)
 *   device label (32)
 *   device power cycles (2)
 *   sensor 0 recorded value (2)
 *   dmx personality (1)
 */

// use from Arduino ! #include "EEPROM.h"

#define DEBUG 1

#include "debug.h"
#include "WidgetSettings.h"
//FIXME: use Progmem/Flash for const strings
const int WidgetSettingsClass::MAGIC_NUMBER = 0x4f4d;
//const long WidgetSettingsClass::DEFAULT_SERIAL_NUMBER = 0x123;
const char WidgetSettingsClass::DEFAULT_LABEL[] = "Def";
...
/*
const WidgetSettings::rdm_personality WidgetSettings::rdm_personalities[] = {
  {1, 6, "6x PWM"},
...
unsigned int WidgetSettingsClass::DevicePowerCycles() const {
  return ReadInt(DEVICE_POWER_CYCLES_OFFSET);
}


void WidgetSettingsClass::SetDevicePowerCycles(unsigned int count) {
  WriteInt(DEVICE_POWER_CYCLES_OFFSET, count);
}
...
unsigned long WidgetSettingsClass::ReadLong(unsigned int offset) const {
...
void WidgetSettingsClass::WriteLong(unsigned int offset, long data) {
