...
// I use 0x2345, because it seems to be unused to avoid conflicts.
...
byte _devID[6] = { 0x23, 0x45, 0x20, 0x12, 0x11, 0x02 };
...
// (Re)Initialize the specified DMX mode.
// For RDM Modes use the extended initRDM method.
// The mode parameter must be a value from enum DMXMode.
void DMXSerialClass2::init (int mode)
{
  _baseInit();
  ...
  if (_dmxMode == DMXController) {
    ...
    _dmxSendBuffer = _dmxData;
    _dmxSendLen = _dmxMaxChannel+1;
    ...
    _dmxMode = DMXError; // not initialized correctly
  } // if
} // init()
...
// For RDM Modes use the extended initRDM method.
// The mode parameter must be a value from enum DMXMode.
void DMXSerialClass2::initRDM(int mode, int footprint, const char* manufacturerLabel, const char* deviceModel, RDMCallbackFunction func )
{
...
void DMXSerialClass2::maxChannel(int channel)
{
...
{
  uint8_t val = 0;
  if ((channel >= 0) && (channel < _footprint)) {
    // in range !
...
uint16_t DMXSerialClass2::getFootprint() { return(_footprint); }
...
void DMXSerialClass2::term(void)
{
  ...
  UCSRnB = 0;
  _dmxMode = DMXNone; // not initialized correctly
} // term()
...
void DMXSerialClass2::_processRDMMessage(byte CmdClass, uint16_t Parameter, boolean handled)
{
  ...
  if (! handled ) {
    if (Parameter == SWAPINT(E120_IDENTIFY_DEVICE)) { // 0x1000
      ...
      devInfo->footprint = SWAPINT(_footprint);
      ...
      _rdm.packet.DataLength = strlen(_manufacturerLabel);
      memcpy(_rdm.packet.Data, _manufacturerLabel, _rdm.packet.DataLength);
      handled = true;
      ...
      _rdm.packet.DataLength = strlen(_deviceModel);
      memcpy(_rdm.packet.Data, _deviceModel, _rdm.packet.DataLength);
      handled = true;
...
ISR(USART_TX_vect)
{
  if ((_dmxMode == DMXController) && (_dmxPos == -1)) {
...
// This interrupt occurs after the start bit of the previous data byte was sent.
ISR(USART_UDRE_vect)
{
  _DMXSerialWriteByte(_dmxData[_dmxPos++]);

  if (_dmxPos > _dmxMaxChannel) {
    _dmxPos = -1; // this series is done. Next time: restart with break.
