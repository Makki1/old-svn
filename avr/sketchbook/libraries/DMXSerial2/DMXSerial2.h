...
// Mode of Operation
typedef enum { 
  DMXNone,       // unspecified, uninitialized
  DMXError,      // error while initializing
  
  DMXController, // always sending
  DMXReceiver,   // always listening
  
  RDMDeviceMode    // currently implemented RDM functionality for a RDM device
} DMXMode;
...
{
  public:
    // Initialize for DMX modes.
    void    init       (int mode);

    // Initialize for RDM modes.
    void    initRDM    (int mode, int footprint, const char* manufacturerLabel, const char* deviceModel, RDMCallbackFunction func );
